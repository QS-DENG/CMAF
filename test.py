...
parser.add_argument('--resume', '-r', default='llcm_deen_p4_n6_lr_0.1_seed_0amp_best.t', type=str, help='resume from checkpoint')
...
if dataset == 'sysu':
    data_path = '/home/share/qsdeng/datasets/sysu/sysu/'
    n_class = 395
    test_mode = [1, 2]
    pool_dim = 2048
elif dataset =='regdb':
...
if dataset == 'llcm':
    ...
    for trial in range(10):
        test_trial = trial +1
        model_path = checkpoint_path + 'regdb_deen_p4_n6_lr_0.1_seed_0_trial_{}mINP_best.t'.format(test_trial)
        if os.path.isfile(model_path):
            print('==> loading checkpoint {}'.format(args.resume))
            checkpoint = torch.load(model_path)
            net.load_state_dict(checkpoint['net'])
