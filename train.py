...
if dataset == 'sysu':
    data_path = '/remote-home/cs_iot_wm/DEAN/datasets/SYSUMM011/'
    log_path = args.log_path + 'sysu_log/'
    test_mode = [1, 2]  # thermal to visible
    pool_dim = 2048
elif dataset == 'regdb':
    data_path = '/home/share/qsdeng/datasets/RegDB/'
    log_path = args.log_path + 'regdb_log/'
    test_mode = [2, 1]  # visible to thermal
    pool_dim = 1024
elif dataset == 'llcm':
    data_path = '/home/share/qsdeng/datasets/LLCM/'
...
criterion_DCL = DCL(num_pos=args.num_pos, feat_norm='no')
...
def train(epoch):
    ...
    if epoch <= flag1:
        c1 = 0.1
    elif epoch > flag1 and epoch <= flag2:
        c1 = 0.01

    for batch_idx, (input1, input2, label1, label2) in enumerate(trainloader):

        lab = torch.cat((label1, label2), 0)
        ...
        if epoch <= flag2:
            loss_dcl1 = criterion_DCL(ft1, lab)

            loss_dcl2 = criterion_DCL(ft2, lab)

            loss_dcl3 = criterion_DCL(ft3, lab)

            loss_dcl = loss_dcl1 + loss_dcl2 + loss_dcl3

            loss_dcl = loss_dcl * c1
        else:
            dtype = loss_id.dtype
            loss_dcl = torch.tensor(0.0, device=device, dtype=dtype)

        loss = loss_id + loss_tri + loss_cpm + loss_ort + loss_dcl
        optimizer.zero_grad()
        loss.backward()
        optimizer.step()
        ...
        if batch_idx % 50 == 0:
            print('Epoch: [{}][{}/{}] '
                  ...
                  'DLoss:{dcl_loss.val:.3f} '
                  'CLoss:{cpm_loss.val:.3f} '
                  'OLoss:{ort_loss.val:.3f} '.format(
                epoch, batch_idx, len(trainloader),
                train_loss=train_loss, id_loss=id_loss, tri_loss=tri_loss, dcl_loss=dcl_loss, cpm_loss=cpm_loss, ort_loss=ort_loss))
...
for epoch in range(start_epoch, 151 - start_epoch):
    ...
    if epoch > 0 and epoch % 1 == 0:
        ...
        if mINP7 > best_mINP:  # not the real best for sysu-mm01
            best_mINP = mINP7
            best_epoch = epoch
            state = {
                'net': net.state_dict(),
            ...
            if cmc7[0] < best_acc:
                torch.save(state, checkpoint_path + suffix + 'mINP_best.t')

        if mAP7 > best_amp:  # not the real best for sysu-mm01
            best_amp = mAP7
            best_epoch = epoch
            state = {
                'net': net.state_dict(),
            ...
            if cmc7[0] < best_acc and mINP7 < best_mINP:
                torch.save(state, checkpoint_path + suffix + 'amp_best.t')
